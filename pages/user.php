...
/*
 * FILE: user.php
 * PURPOSE: Dashboard for users with role 'user'.
 * LAYOUT:  Collapsible sidebar + empty content area (.showcase).
 * SHARED:  main_interface.css, behavior.js (same as admin.php, techn.php).
 */

// ── Session guard ──
session_start();

// LOGIC: No session email → visitor is not logged in.
if (!isset($_SESSION['email'])) {
    header("Location: ../pages/login_signup.php");
    exit();
}

// LOGIC: Wrong role → block access to this dashboard.
...
<body>
    ...
    <!-- LOGIC: Flex row — sidebar (.main-head) + content (.showcase) side by side -->
    <main class="main-wrap">

        <!-- ── SIDEBAR ── -->
        <!-- LOGIC: behavior.js toggles "active" class → CSS collapses to 60px -->
        <header class="main-head">
            <div class="main-nav">
                <nav class="navbar">
                    <div class="navbar-nav">

                        <!-- Logo (expanded) + hamburger button (collapsed) -->
                        <div class="logo">
                            <img src="../images/ZPGC.com2.png" alt="ZPGC Logo">
                            <!-- LOGIC: .showcase-toggler click target for behavior.js -->
                        ...
                        <!-- ── NAV ITEMS (user role) ── -->
                        <!-- SYNTAX: Each item = <li> → <a class="nav-link"> → icon + <span class="link-text"> -->
                        <!-- LOGIC: .link-text hidden when sidebar collapsed (CSS) -->
                        <ul class="nav-list">
                            ...
                            <!-- LOGIC: Last item — CSS margin-top:auto pins to bottom of sidebar -->
                            <li class="nav-list-item">
        ...
        <!-- ── MAIN CONTENT ── -->
        <!-- LOGIC: flex:1 fills remaining width; placeholder for future ticket/dashboard UI -->
